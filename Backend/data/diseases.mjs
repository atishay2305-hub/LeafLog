...
export const getAllCommonNames = async () => {
    try {
        ...
        const diseases = await diseasesCollectionRef.find({}, { common_name: 1 }).toArray();
...
export const getAllScientificNames = async () => {
    try {
        const diseasesCollectionRef = await diseasesCollection();
        const diseases = await diseasesCollectionRef.find({}, { scientific_name: 1 }).toArray();
...
export const getAllDescriptions = async () => {
    try {
        const diseasesCollectionRef = await diseasesCollection();
        const diseases = await diseasesCollectionRef.find({}, { description: 1 }).toArray();
...
export const getAllHosts = async () => {
    try {
        const diseasesCollectionRef = await diseasesCollection();
        const diseases = await diseasesCollectionRef.find({}, { host: 1 }).toArray();
...
export const getAllOtherNames = async () => {
    try {
        const diseasesCollectionRef = await diseasesCollection();
        const diseases = await diseasesCollectionRef.find({}, { other_name: 1 }).toArray();
        const otherNames = diseases.map(disease => disease.other_name || "there is no other name");
        return otherNames;
    } catch (error) {
        console.error('Error:', error.message);
        return [];
